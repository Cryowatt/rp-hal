...
impl Inner {
    ...
    fn ep_allocate(
        ...
        let max_packet_size_limit = match ep_type {
            EndpointType::Isochronous { synchronization: _, usage: _ } => 1023,
            _ => 64
        };
    ...
    fn ep_reset_all(&mut self) {
        ...
        .filter_map(|(i, ep)| ep.as_ref().map(|ep| (i, ep)))
        {
            ...
            let ep_type = match ep.ep_type {
                ...
                EndpointType::Isochronous { synchronization: _, usage: _ } => ENDPOINT_TYPE_A::ISOCHRONOUS,
